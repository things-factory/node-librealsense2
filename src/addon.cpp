...
public:
  ...
  v8::Local<v8::Value> GetMember(const char *name) const
  {
    ...
    return v8obj_->Get(context, Nan::New(name).ToLocalChecked()).ToLocalChecked();
  }
  ...
  {
    v8::Local<v8::Context> context = v8::Isolate::GetCurrent()->GetCurrentContext();
    v8obj_->Set(context, Nan::New(name).ToLocalChecked(), Nan::Undefined());
  }

  void DeleteMember(const char *name)
  {
    v8::Local<v8::Context> context = v8::Isolate::GetCurrent()->GetCurrentContext();
    v8obj_->Delete(context, Nan::New(name).ToLocalChecked());
  }

  void SetMember(const char *name, const char *value)
  {
    v8::Local<v8::Context> context = v8::Isolate::GetCurrent()->GetCurrentContext();
    v8obj_->Set(context, Nan::New(name).ToLocalChecked(),
  ...
  {
    v8::Local<v8::Context> context = v8::Isolate::GetCurrent()->GetCurrentContext();
    v8obj_->Set(context, Nan::New(name).ToLocalChecked(), value);
  }
  ...
  {
    v8obj_->Set(Nan::GetCurrentContext(), Nan::New(name).ToLocalChecked(), Nan::New(value));
  }

  bool IsMemberPresent(const char *name) const
  {
    v8::Local<v8::Context> context = v8::Isolate::GetCurrent()->GetCurrentContext();
    v8::MaybeLocal<v8::Value> v = v8obj_->Get(context, Nan::New(name).ToLocalChecked());
    return !v.IsEmpty();
  }
...
private:
  ...
  {
    if (singleton_)
      uv_mutex_lock(&(singleton_->mutex_));
  }
  static void Unlock()
  {
    if (singleton_)
      uv_mutex_unlock(&(singleton_->mutex_));
  }
  static void AsyncProc(uv_async_t *async)
  {
    ...
    if (MainThreadCallbackInfo::InfoExist(info))
      delete info;
  }
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    exports->Set(context, Nan::New("RSStreamProfile").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    error_ = nullptr;
    if (profile_ && own_profile_)
      rs2_delete_stream_profile(profile_);
    profile_ = nullptr;
  }
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSFrame").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
                               ...
                               const rs2_pose &pose)
  {
    ...
    auto angular_velocity_obj = Nan::GetRealNamedProperty(
                                    obj, angular_velocity_name)
                                    .ToLocalChecked()
                                    ->ToObject(context)
                                    .ToLocalChecked();
    auto angular_acceleration_obj = Nan::GetRealNamedProperty(
                                        obj, angular_acceleration_name)
                                        .ToLocalChecked()
                                        ->ToObject(context)
                                        .ToLocalChecked();
  ...
  static NAN_METHOD(GetFrameMetadata)
  {
    ...
    rs2_frame_metadata_value metadata =
        (rs2_frame_metadata_value)(info[0]->IntegerValue(context).ToChecked());
  ...
  static NAN_METHOD(ExportToPly)
  {
    ...
    auto texture = Nan::ObjectWrap::Unwrap<RSFrame>(info[1]->ToObject(context).ToLocalChecked());
    info.GetReturnValue().Set(Nan::Undefined());
    if (!me || !texture)
      return;
  ...
  static NAN_METHOD(GetDistance)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto me = Nan::ObjectWrap::Unwrap<RSFrame>(info.Holder());
    auto x = info[0]->IntegerValue(context).ToChecked();
    auto y = info[1]->IntegerValue(context).ToChecked();
    if (!me)
      return;
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSFrameQueue").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static NAN_METHOD(WaitForFrame)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    int32_t timeout = info[0]->IntegerValue(context).ToChecked(); // in ms
  ...
  static NAN_METHOD(Create)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    int32_t capacity = info[0]->IntegerValue(context).ToChecked();
    auto me = Nan::ObjectWrap::Unwrap<RSFrameQueue>(info.Holder());
    if (!me)
      return;
...
{
public:
  FrameCallbackInfo(rs2_frame *frame, void *data) : frame_(frame), sensor_(static_cast<RSSensor *>(data)) {}
...
public:
  NotificationCallbackInfo(const char *desc,
                           ...
                           RSSensor *s) : desc_(desc), time_(time), severity_(severity),
...
{
public:
  explicit FrameCallbackForProcessingBlock(rs2_processing_block *block_ptr) : block_(block_ptr), error_(nullptr) {}
...
{
public:
  PlaybackStatusCallbackInfo(rs2_playback_status status, RSDevice *dev) : status_(status), dev_(dev), error_(nullptr) {}
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSFrameSet").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static NAN_METHOD(GetFrame)
  {
    ...
    rs2_stream stream = static_cast<rs2_stream>(info[0]->IntegerValue(context).ToChecked());
    auto stream_index = info[1]->IntegerValue(context).ToChecked();
  ...
  static NAN_METHOD(ReplaceFrame)
  {
    ...
    rs2_stream stream = static_cast<rs2_stream>(info[0]->IntegerValue(context).ToChecked());
    auto stream_index = info[1]->IntegerValue(context).ToChecked();
    auto target_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[2]->ToObject(context).ToLocalChecked());
  ...
  static NAN_METHOD(IndexToStream)
  {
    ...
    int32_t index = info[0]->IntegerValue(context).ToChecked();
    rs2_frame *frame = GetNativeResult<rs2_frame *>(rs2_extract_frame,
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSSyncer").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    error_ = nullptr;
    if (syncer_)
      rs2_delete_processing_block(syncer_);
    syncer_ = nullptr;
    if (frame_queue_)
      rs2_delete_frame_queue(frame_queue_);
    frame_queue_ = nullptr;
  ...
  static NAN_METHOD(WaitForFrames)
  {
    ...
    auto frameset = Nan::ObjectWrap::Unwrap<RSFrameSet>(info[0]->ToObject(context).ToLocalChecked());
    auto timeout = info[1]->IntegerValue(context).ToChecked();
    if (!me || !frameset)
      return;
...
public:
      ...
      const Nan::FunctionCallbackInfo<v8::Value> &info)
  {
    ...
    int32_t option = info[0]->IntegerValue(context).ToChecked();
      ...
      const Nan::FunctionCallbackInfo<v8::Value> &info)
  {
    ...
    int32_t option = info[0]->IntegerValue(context).ToChecked();
    auto val = info[1]->NumberValue(context).ToChecked();
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSSensor").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    error_ = nullptr;
    if (sensor_)
      rs2_delete_sensor(sensor_);
    sensor_ = nullptr;
    if (profile_list_)
      rs2_delete_stream_profiles_list(profile_list_);
  ...
  static NAN_METHOD(SupportsOption)
  {
    ...
    if (me)
      return me->SupportsOptionInternal(info);
  ...
  static NAN_METHOD(GetOption)
  {
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (me)
      return me->GetOptionInternal(info);
  ...
  static NAN_METHOD(GetOptionDescription)
  {
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (me)
      return me->GetOptionDescriptionInternal(info);
  ...
  static NAN_METHOD(GetOptionValueDescription)
  {
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (me)
      return me->GetOptionValueDescriptionInternal(info);
  ...
  static NAN_METHOD(SetOption)
  {
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (me)
      return me->SetOptionInternal(info);
  ...
  static NAN_METHOD(GetOptionRange)
  {
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (me)
      return me->GetOptionRangeInternal(info);
  ...
  static NAN_METHOD(IsOptionReadonly)
  {
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (me)
      return me->IsOptionReadonlyInternal(info);
  ...
  static NAN_METHOD(GetCameraInfo)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    int32_t camera_info = info[0]->IntegerValue(context).ToChecked();
    ;
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (!me)
      return;
  ...
  static NAN_METHOD(StartWithSyncer)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto syncer = Nan::ObjectWrap::Unwrap<RSSyncer>(info[0]->ToObject(context).ToLocalChecked());
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (!me || !syncer)
      return;
  ...
  static NAN_METHOD(StartWithCallback)
  {
    ...
    auto frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[1]->ToObject(context).ToLocalChecked());
    auto depth_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[2]->ToObject(context).ToLocalChecked());
    auto video_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[3]->ToObject(context).ToLocalChecked());
    auto disparity_frame = Nan::ObjectWrap::Unwrap<RSFrame>(
        info[4]->ToObject(context).ToLocalChecked());
    auto motion_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[5]->ToObject(context).ToLocalChecked());
    auto pose_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[6]->ToObject(context).ToLocalChecked());
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
  ...
  static NAN_METHOD(OpenStream)
  {
    ...
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    auto profile =
        Nan::ObjectWrap::Unwrap<RSStreamProfile>(info[0]->ToObject(context).ToLocalChecked());
    if (!me || !profile)
      return;
  ...
  static NAN_METHOD(OpenMultipleStream)
  {
    ...
    for (uint32_t i = 0; i < len; i++)
    {
      auto profile = Nan::ObjectWrap::Unwrap<RSStreamProfile>(array->Get(context, i).ToLocalChecked()->ToObject(context).ToLocalChecked());
  ...
  static NAN_METHOD(SetRegionOfInterest)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    int32_t minx = info[0]->IntegerValue(context).ToChecked();
    int32_t miny = info[1]->IntegerValue(context).ToChecked();
    int32_t maxx = info[2]->IntegerValue(context).ToChecked();
    int32_t maxy = info[3]->IntegerValue(context).ToChecked();
    auto me = Nan::ObjectWrap::Unwrap<RSSensor>(info.Holder());
    if (!me)
      return;
  ...
  static NAN_METHOD(Is)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    int32_t stype = info[0]->IntegerValue(context).ToChecked();
...
public:
  ...
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSDevice").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static NAN_METHOD(SpawnRecorderDevice)
  {
    ...
    v8::String::Utf8Value file(isolate, info[0]);
  ...
  static NAN_METHOD(GetPosition)
  {
    ...
    auto pos = static_cast<uint32_t>(GetNativeResult<uint32_t>(
                                         rs2_playback_get_position, &me->error_, me->dev_, &me->error_) /
                                     1000000);
  ...
  static NAN_METHOD(Seek)
  {
    ...
    uint64_t time = info[0]->IntegerValue(context).ToChecked();
  ...
  static NAN_METHOD(SetIsRealTime)
  {
    ...
    auto val = info[0]->BooleanValue(isolate);
  ...
  static NAN_METHOD(SetPlaybackSpeed)
  {
    ...
    auto speed = info[0]->NumberValue(context).ToChecked();
  ...
  static NAN_METHOD(SetStatusChangedCallbackMethodName)
  {
    ...
    v8::String::Utf8Value method(isolate, info[0]);
...
void NotificationCallbackInfo::Run()
{
  ...
  v8::Local<v8::Value> args[1] = {
      RSNotification(
          desc_, time_, severity_, category_, serialized_data_)
          .GetObject()};
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSPointCloud").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    if (processing_block_)
      rs2_delete_processing_block(processing_block_);
  ...
  static NAN_METHOD(Calculate)
  {
    ...
    auto me = Nan::ObjectWrap::Unwrap<RSPointCloud>(info.Holder());
    auto frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[0]->ToObject(context).ToLocalChecked());
    auto target_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[1]->ToObject(context).ToLocalChecked());
  ...
  static NAN_METHOD(MapTo)
  {
    ...
    CallNativeFunc(rs2_set_option, &me->error_,
                   ...
                   &me->error_);
    if (extrator.stream_ == RS2_STREAM_DEPTH)
      return;
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSDeviceList").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static NAN_METHOD(Contains)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto me = Nan::ObjectWrap::Unwrap<RSDeviceList>(info.Holder());
    auto dev = Nan::ObjectWrap::Unwrap<RSDevice>(info[0]->ToObject(context).ToLocalChecked());
    if (!me && dev)
      return;
  ...
  static NAN_METHOD(GetDevice)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto me = Nan::ObjectWrap::Unwrap<RSDeviceList>(info.Holder());
    auto index = info[0]->IntegerValue(context).ToChecked();
    if (!me)
      return;
...
public:
  ...
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSContext").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static void New(const Nan::FunctionCallbackInfo<v8::Value> &info)
  {
    ...
    if (info.Length())
    {
      v8::String::Utf8Value type_str(isolate, info[0]);
    ...
    {
      v8::String::Utf8Value file(isolate, info[1]);
      v8::String::Utf8Value section(isolate, info[2]);
      obj->file_name_ = std::string(*file);
      obj->section_ = std::string(*section);
    }
    if (type == kRecording)
      obj->mode_ = static_cast<rs2_recording_mode>(info[3]->IntegerValue(context).ToChecked());
    obj->Wrap(info.This());
    info.GetReturnValue().Set(info.This());
  }
  ...
  static NAN_METHOD(LoadDeviceFile)
  {
    ...
    auto device_file = info[0]->ToString(context).ToLocalChecked();
    v8::String::Utf8Value value(isolate, device_file);
  ...
  static NAN_METHOD(CreateDeviceFromSensor)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto sensor = Nan::ObjectWrap::Unwrap<RSSensor>(info[0]->ToObject(context).ToLocalChecked());
    if (!sensor)
      return;
...
public:
  DevicesChangedCallbackInfo(rs2_device_list *r,
                             rs2_device_list *a, RSContext *ctx) : removed_(r), added_(a), ctx_(ctx) {}
...
private:
  ...
  static void New(const Nan::FunctionCallbackInfo<v8::Value> &info)
  {
    ...
    if (info.IsConstructCall())
    {
      ...
      RSContext *ctx = Nan::ObjectWrap::Unwrap<RSContext>(info[0]->ToObject(context).ToLocalChecked());
      obj->ctx_ = ctx->ctx_;
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSConfig").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static NAN_METHOD(EnableStream)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto me = Nan::ObjectWrap::Unwrap<RSConfig>(info.Holder());
    auto stream = info[0]->IntegerValue(context).ToChecked();
    auto index = info[1]->IntegerValue(context).ToChecked();
    auto width = info[2]->IntegerValue(context).ToChecked();
    auto height = info[3]->IntegerValue(context).ToChecked();
    auto format = info[4]->IntegerValue(context).ToChecked();
    auto framerate = info[5]->IntegerValue(context).ToChecked();
    if (!me || !me->config_)
      return;
  ...
  static NAN_METHOD(EnableDeviceFromFileRepeatOption)
  {
    ...
    auto device_file = info[0]->ToString(context).ToLocalChecked();
    auto repeat = info[1]->BooleanValue(isolate);
    v8::String::Utf8Value value(isolate, device_file);
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSPipeline").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  static NAN_METHOD(Create)
  {
    ...
    info.GetReturnValue().Set(Nan::Undefined());
    auto me = Nan::ObjectWrap::Unwrap<RSPipeline>(info.Holder());
    auto rsctx = Nan::ObjectWrap::Unwrap<RSContext>(info[0]->ToObject(context).ToLocalChecked());
    if (!me || !rsctx)
      return;
  ...
  static NAN_METHOD(StartWithConfig)
  {
    ...
    RSConfig *config = Nan::ObjectWrap::Unwrap<RSConfig>(info[0]->ToObject(context).ToLocalChecked());
...
NAN_METHOD(RSConfig::Resolve)
{
  ...
  RSPipeline *pipe = Nan::ObjectWrap::Unwrap<RSPipeline>(info[0]->ToObject(context).ToLocalChecked());
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSColorizer").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    error_ = nullptr;
    if (colorizer_)
      rs2_delete_processing_block(colorizer_);
    colorizer_ = nullptr;
    if (frame_queue_)
      rs2_delete_frame_queue(frame_queue_);
    frame_queue_ = nullptr;
  ...
  static NAN_METHOD(Colorize)
  {
    ...
    info.GetReturnValue().Set(Nan::False());
    auto me = Nan::ObjectWrap::Unwrap<RSColorizer>(info.Holder());
    RSFrame *depth = Nan::ObjectWrap::Unwrap<RSFrame>(info[0]->ToObject(context).ToLocalChecked());
    RSFrame *target = Nan::ObjectWrap::Unwrap<RSFrame>(info[1]->ToObject(context).ToLocalChecked());
...
public:
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSAlign").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    error_ = nullptr;
    if (align_)
      rs2_delete_processing_block(align_);
    align_ = nullptr;
    if (frame_queue_)
      rs2_delete_frame_queue(frame_queue_);
    frame_queue_ = nullptr;
  ...
  static void New(const Nan::FunctionCallbackInfo<v8::Value> &info)
  {
    ...
    auto stream = static_cast<rs2_stream>(info[0]->IntegerValue(context).ToChecked());
  ...
  static NAN_METHOD(Process)
  {
    ...
    auto me = Nan::ObjectWrap::Unwrap<RSAlign>(info.Holder());
    auto frameset = Nan::ObjectWrap::Unwrap<RSFrameSet>(info[0]->ToObject(context).ToLocalChecked());
    auto target_fs = Nan::ObjectWrap::Unwrap<RSFrameSet>(info[1]->ToObject(context).ToLocalChecked());
...
public:
  ...
  static void Init(v8::Local<v8::Object> exports)
  {
    ...
    constructor_.Reset(tpl->GetFunction(context).ToLocalChecked());
    exports->Set(context, Nan::New("RSFilter").ToLocalChecked(), tpl->GetFunction(context).ToLocalChecked());
  }
...
private:
  ...
  void DestroyMe()
  {
    ...
    error_ = nullptr;
    if (block_)
      rs2_delete_processing_block(block_);
    block_ = nullptr;
    if (frame_queue_)
      rs2_delete_frame_queue(frame_queue_);
    frame_queue_ = nullptr;
  ...
  static NAN_METHOD(Process)
  {
    ...
    auto me = Nan::ObjectWrap::Unwrap<RSFilter>(info.Holder());
    auto input_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[0]->ToObject(context).ToLocalChecked());
    auto out_frame = Nan::ObjectWrap::Unwrap<RSFrame>(info[1]->ToObject(context).ToLocalChecked());
    info.GetReturnValue().Set(Nan::False());
    if (!me || !input_frame || !out_frame)
      return;
