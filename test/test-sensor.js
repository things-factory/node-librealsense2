...
describe("Sensor test", function () {
    ...
    sensors.forEach((sensor) => {
      Object.keys(rs2.option).forEach((o) => {
        ...
        if (sensor.supportsOption(rs2.option[o])) {
          assert.equal(
            typeof sensor.isOptionReadOnly(rs2.option[o]),
            "boolean"
          );
        } else {
          assert.equal(
    ...
    sensors.forEach((sensor) => {
      Object.keys(rs2.option).forEach((o) => {
        ...
        if (sensor.supportsOption(rs2.option[o])) {
          ...
          assert.equal(
            typeof sensor.getOptionRange(rs2.option[o]),
            "undefined"
          );
        }
      });
    ...
    sensors.forEach((sensor) => {
      ...
      Object.keys(rs2.option).forEach((o) => {
        ...
        if (o.toUpperCase() === "OPTION_DEPTH_UNITS") {
          assert.equal(
            Math.round(vNew * 100000) / 100000,
            Math.round(vSet * 100000) / 100000
          );
        } else {
  ...
  it("Testing method getOptionDescription", () => {
    sensors.forEach((sensor) => {
      optionsTestArray.forEach((o) => {
        assert(
          typeof sensor.getOptionDescription(o) === "string" ||
  ...
  it("Testing method getOptionValueDescription", () => {
    sensors.forEach((sensor) => {
      optionsTestArray.forEach((o) => {
        ...
        assert(
          typeof sensor.getOptionValueDescription(o, value) === "undefined" ||
  ...
  it("Testing method open, profileArray", () => {
    ...
    for (let i in profiles) {
      let strings =
        "" +
        profiles[i].widthValue +
        profiles[i].heightValue +
        profiles[i].fps +
        profiles[i].formatValue;
    ...
    function testSingleProfile(sensor, profile) {
      ...
      return new Promise((resolve, reject) => {
        sensor.start((frame) => {
          ...
          assert.equal(
            Object.prototype.toString.call(frame.data),
            expectDataType
          );
