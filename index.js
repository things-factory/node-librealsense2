...
const RS2 = require("bindings")("node-librealsense2");
...
class Device {
  ...
  getCameraInfo(info) {
    ...
    if (arguments.length === 0) {
      ...
      if (
        this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_SERIAL_NUMBER)
      ) {
        result.serialNumber = this.cxxDev.getCameraInfo(
          camera_info.CAMERA_INFO_SERIAL_NUMBER
        );
      }
      if (
        this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_FIRMWARE_VERSION)
      ) {
      ...
      if (
        this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_PHYSICAL_PORT)
      ) {
        result.physicalPort = this.cxxDev.getCameraInfo(
          camera_info.CAMERA_INFO_PHYSICAL_PORT
        );
      }
      if (
        this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_DEBUG_OP_CODE)
      ) {
        result.debugOpCode = this.cxxDev.getCameraInfo(
          camera_info.CAMERA_INFO_DEBUG_OP_CODE
        );
      }
      if (
        this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_ADVANCED_MODE)
      ) {
        result.advancedMode = this.cxxDev.getCameraInfo(
          camera_info.CAMERA_INFO_ADVANCED_MODE
        );
      }
      if (this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_PRODUCT_ID)) {
        result.productId = this.cxxDev.getCameraInfo(
          camera_info.CAMERA_INFO_PRODUCT_ID
        );
      }
      if (
        this.cxxDev.supportsCameraInfo(camera_info.CAMERA_INFO_CAMERA_LOCKED)
      ) {
        result.cameraLocked = this.cxxDev.getCameraInfo(
          camera_info.CAMERA_INFO_CAMERA_LOCKED
        );
      }
      if (
        this.cxxDev.supportsCameraInfo(
      ...
      const val = checkArgumentType(
        arguments,
        constants.camera_info,
        0,
        funcName
      );
      return this.cxxDev.supportsCameraInfo(val)
        ? this.cxxDev.getCameraInfo(val)
        : undefined;
    }
  }
...
class Sensor extends Options {
  ...
  getCameraInfo(info) {
    ...
    const i = checkArgumentType(arguments, constants.camera_info, 0, funcName);
    return this.cxxSensor.supportsCameraInfo(i)
      ? this.cxxSensor.getCameraInfo(i)
      : undefined;
  }
  ...
  start(callback) {
    ...
    if (arguments[0] instanceof Syncer) {
      ...
      this.cxxSensor.startWithCallback(
        "frameCallback",
        this.frame.cxxFrame,
        this.depthFrame.cxxFrame,
        this.videoFrame.cxxFrame,
        this.disparityFrame.cxxFrame,
        this.motionFrame.cxxFrame,
        this.poseFrame.cxxFrame
      );
    }
  }
  ...
  setNotificationsCallback(callback) {
    ...
    if (!this.cxxSensor.notificationCallback) {
      this.cxxSensor.notificationCallback = function (info) {
        ...
        info.category = notification_category.notificationCategoryToString(
          info.category
        );
...
class RecorderDevice extends Device {
  ...
  static from(device) {
    return device.cxxDev.isRecorder()
      ? new RecorderDevice(null, null, device.cxxDev, false)
      : undefined;
  }
  ...
  constructor(file, device, cxxDev = undefined, autoDelete = true) {
    ...
    super(
      cxxDev ? cxxDev : device.cxxDev.spawnRecorderDevice(file),
      autoDelete
    );
  }
  /**
...
class PlaybackDevice extends Device {
  ...
  static from(device) {
    return device.cxxDev.isPlayback()
      ? new PlaybackDevice(device.cxxDev, false)
      : undefined;
  }
  ...
  get currentStatus() {
    ...
    return {
      status: cxxStatus,
      description: playback_status.playbackStatusToString(cxxStatus),
    };
  }
  ...
  setStatusChangedCallback(callback) {
    ...
    this._events.on("status-changed", (status) => {
      callback({
        status: status,
        description: playback_status.playbackStatusToString(status),
      });
    });
    let inst = this;
...
class PointCloud extends Options {
  ...
  calculate(depthFrame) {
    ...
    if (
      this.cxxPointCloud.calculate(
        depthFrame.cxxFrame,
        this.pointsFrame.cxxFrame
      )
    ) {
...
class Colorizer extends Options {
  ...
  colorize(depthFrame) {
    ...
    checkArgumentType(arguments, Frame, 0, funcName);
    const success = this.cxxColorizer.colorize(
      depthFrame.cxxFrame,
      this.depthRGB.cxxFrame
    );
...
class Align {
  ...
  process(frameSet) {
    ...
    if (
      this.cxxAlign.process(frameSet.cxxFrameSet, this.frameSet.cxxFrameSet)
    ) {
...
class Frame {
  ...
  updateProfile() {
    ...
    if (this.cxxFrame) {
      ...
      if (cxxProfile) {
        this.streamProfile = StreamProfile._internalCreateStreamProfile(
          cxxProfile
        );
      }
    }
  ...
  get width() {
    return this.streamProfile.width
      ? this.streamProfile.width
      : this.cxxFrame.getWidth();
  }

  get height() {
    return this.streamProfile.height
      ? this.streamProfile.height
      : this.cxxFrame.getHeight();
  }
  ...
  frameMetadata(metadata) {
    ...
    checkArgumentLength(1, 1, arguments.length, funcName);
    const m = checkArgumentType(
      arguments,
      constants.frame_metadata,
      0,
      funcName
    );
  ...
  get textureCoordinates() {
    ...
    if (this.cxxFrame.canGetPoints()) {
      ...
      if (
        ...
        newLength !== this.textureCoordData.byteLength
      ) {
...
class FrameSet {
  ...
  __internalGetFrameCache(stream, streamIndex, callback) {
    ...
    if (idx === undefined) {
      ...
      if (
        !this.cxxFrameSet.replaceFrame(
          this.cacheMetadata[idx].stream,
          streamIndex,
          frame.cxxFrame
        )
      ) {
  ...
  getFrame(stream, streamIndex = 0) {
    ...
    return this.__internalGetFrameCache(
      s,
      streamIndex,
      this.__internalGetFrame.bind(this)
    );
  }
  ...
  start() {
    ...
    if (arguments.length === 0) {
      ...
      return new PipelineProfile(
        this.cxxPipeline.startWithConfig(arguments[0].cxxConfig)
      );
    }
  }
...
class Config {
  ...
  resolve(pipeline) {
    ...
    return new PipelineProfile(
      this.cxxConfig.resolve(arguments[0].cxxPipeline)
    );
  }
...
class Filter extends Options {
  ...
  process(frame) {
    ...
    if (
      this.cxxObj &&
      this.cxxObj.process(frame.cxxFrame, this.frame.cxxFrame)
    ) {
...
function checkArgumentLength(minLengh, maxLength, actualLength, funcName) {
  ...
  if (minLengh === maxLength) {
    ...
    throw new TypeError(
      funcName + " expects " + minLengh + "~" + maxLength + " arguments"
    );
  }
}
      ...
      if (typeof expectedType === "function") {
        ...
        for (let mem in constants) {
          if (constants[mem] === expectedType) {
            return checkEnumObjectArgument(
              args,
              expectedType,
              argIndex,
              funcName,
...
function checkEnumObjectArgument(
  args,
  expectedType,
  argIndex,
  funcName,
  ...
  switch (expectedType) {
    ...
    case constants.notification_category:
      rangeStart =
        constants.notification_category.NOTIFICATION_CATEGORY_FRAMES_TIMEOUT;
    ...
    case constants.sr300_visual_preset:
      rangeStart =
        constants.sr300_visual_preset.SR300_VISUAL_PRESET_SHORT_RANGE;
...
function checkNumberRange(arg, start, end, argIndex, funcName) {
  if (arg < start || arg > end) {
    throw new TypeError(
      "argument " + argIndex + " of " + funcName + " is invalid"
    );
  }
}
...
function checkDiscreteArgumentLength(lenthArray, actualLength, funcName) {
  if (lenthArray.indexOf(actualLength) === -1) {
    throw new TypeError(
      funcName + " expects " + lenthArray.toString() + " arguments"
    );
  }
}
...
function equalsToEither(arg, strConst, numConst) {
  return (
    (typeof arg === "string" && arg === strConst) ||
...
util.projectPointToPixel = function (intrinsics, pointCoordinate) {
  ...
  if (
    ...
    throw new TypeError(
      "projectPointToPixel cannot project to an inverse-distorted image"
    );
  }
  ...
  if (
    ...
    const rd =
      (1.0 / intrinsics.coeffs[0]) *
      Math.atan(2 * r * Math.tan(intrinsics.coeffs[0] / 2.0));
    x *= rd / r;
    y *= rd / r;
  }
  return {
    x: x * intrinsics.fx + intrinsics.ppx,
    y: y * intrinsics.fy + intrinsics.ppy,
  };
};
...
util.deprojectPixelToPoint = function (intrinsics, pixelCoordinate, depth) {
  ...
  if (
    ...
    throw new TypeError(
      "deprojectPixelToPoint cannot deproject from a forward-distorted image"
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      "deprojectPixelToPoint cannot deproject to an ftheta image"
    );
  }
...
util.writeFrameToFileAsync = function (path, frame, fileFormat = "png") {
  ...
  if (fileFormat.toLowerCase() === "png") {
    ...
    throw new TypeError(
      "util.writeFrameToFileAsync expects a string as the 3rd argument and only 'png' is supported now."
    ); // eslint-disable-line
  }
};
...
util.fov = function (intrinsics) {
  ...
  let h =
    (Math.atan(ppx + 0.5, fx) + Math.atan(width - ppx - 0.5, fx)) * 57.2957795;
  let v =
    (Math.atan(ppy + 0.5, fy) + Math.atan(height - ppy - 0.5, fy)) * 57.2957795;
  return { h: h, v: v };
};
...
util.writeFrameToFile = function (path, frame, fileFormat = "png") {
  ...
  if (fileFormat.toLowerCase() === "png") {
    ...
    throw new TypeError(
      "util.writeFrameToFile expects a string as the 3rd argument and only 'png' is supported now."
    ); // eslint-disable-line
  }
};
...
function frameMetadataContent(frame) {
  ...
  let content =
    "Stream," +
    stream.streamToString(frame.profile.streamType) +
    "\nMetadata Attribute,Value\n"; // eslint-disable-line
  for (let i = 0; i < frame_metadata.FRAME_METADATA_COUNT; i++) {
    if (frame.supportsFrameMetadata(i)) {
      content +=
        frame_metadata.frameMetadataToString(i) +
        "," +
        frame.frameMetadata(i) +
        "\n";
    }
  }
  return content;
...
const option = {
  ...
  OPTION_HISTOGRAM_EQUALIZATION_ENABLED:
    RS2.RS2_OPTION_HISTOGRAM_EQUALIZATION_ENABLED,
  ...
  OPTION_AUTO_EXPOSURE_CONVERGE_STEP:
    RS2.RS2_OPTION_AUTO_EXPOSURE_CONVERGE_STEP,
  /**
...
const camera_info = {
  ...
  CAMERA_INFO_RECOMMENDED_FIRMWARE_VERSION:
    RS2.RS2_CAMERA_INFO_RECOMMENDED_FIRMWARE_VERSION,
  /**
...
const frame_metadata = {
  ...
  FRAME_METADATA_FRAME_LASER_POWER_MODE:
    RS2.RS2_FRAME_METADATA_FRAME_LASER_POWER_MODE,
  /**
   * Exposure priority.
   * <br>Equivalent to its lowercase counterpart
   * @type {Integer}
  ...
  FRAME_METADATA_EXPOSURE_ROI_BOTTOM:
    RS2.RS2_FRAME_METADATA_EXPOSURE_ROI_BOTTOM,
  /**
   * Color image brightness.
   * <br>Equivalent to its lowercase counterpart
   * @type {Integer}
  ...
  FRAME_METADATA_BACKLIGHT_COMPENSATION:
    RS2.RS2_FRAME_METADATA_BACKLIGHT_COMPENSATION,
  /**
   * Color image hue.
   * <br>Equivalent to its lowercase counterpart
   * @type {Integer}
  ...
  FRAME_METADATA_MANUAL_WHITE_BALANCE:
    RS2.RS2_FRAME_METADATA_MANUAL_WHITE_BALANCE,
  /**
   * Power Line Frequency for anti-flickering Off/50Hz/60Hz/Auto.
   * <br>Equivalent to its lowercase counterpart
   * @type {Integer}
  ...
  FRAME_METADATA_POWER_LINE_FREQUENCY:
    RS2.RS2_FRAME_METADATA_POWER_LINE_FREQUENCY,
  /**
   * Color lowlight compensation. Zero corresponds to switched off.
   * <br>Equivalent to its lowercase counterpart
   * @type {Integer}
  ...
  FRAME_METADATA_LOW_LIGHT_COMPENSATION:
    RS2.RS2_FRAME_METADATA_LOW_LIGHT_COMPENSATION,
  /**
   * Number of enumeration values. Not a valid input: intended to be used in for-loops.
   * @type {Integer}
   */
...
const notification_category = {
  ...
  notificationCategoryToString: function (notification) {
    ...
    const i = checkArgumentType(
      arguments,
      constants.notification_category,
      0,
      funcName
    );
    switch (i) {
...
const sr300_visual_preset = {
  ...
  SR300_VISUAL_PRESET_BACKGROUND_SEGMENTATION:
    RS2.RS2_SR300_VISUAL_PRESET_BACKGROUND_SEGMENTATION,
  /**
  ...
  SR300_VISUAL_PRESET_GESTURE_RECOGNITION:
    RS2.RS2_SR300_VISUAL_PRESET_GESTURE_RECOGNITION,
  /**
  ...
  SR300_VISUAL_PRESET_OBJECT_SCANNING:
    RS2.RS2_SR300_VISUAL_PRESET_OBJECT_SCANNING,
  /**
  ...
  SR300_VISUAL_PRESET_FACE_ANALYTICS:
    RS2.RS2_SR300_VISUAL_PRESET_FACE_ANALYTICS,
  /**
...
const rs400_visual_preset = {
  ...
  RS400_VISUAL_PRESET_MEDIUM_DENSITY:
    RS2.RS2_RS400_VISUAL_PRESET_MEDIUM_DENSITY,
  /**
  ...
  RS400_VISUAL_PRESET_REMOVE_IR_PATTERN:
    RS2.RS2_RS400_VISUAL_PRESET_REMOVE_IR_PATTERN,
  /**
   * Number of enumeration values. Not a valid input: intended to be used in for-loops.
   * @type {Integer}
   */
