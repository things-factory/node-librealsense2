...
let rs2;
try {
  rs2 = require("@things-factory/node-librealsense2");
} catch (e) {
...
if (isRecord || isPlayback) {
  if (!recordFile || typeof recordFile !== "string") {
    throw new TypeError(
      "record/playback file must be provided like'--playback ut_record.data'"
    );
  }
  if (isPlayback && !fs.existsSync(recordFile)) {
    throw new TypeError(
      "the playback file '" + recordFile + "' doesn't exists"
    );
  }
}
...
if (!(isRecord || isPlayback)) {
  describe("Colorizer test", function () {
    ...
    it("colorizer option API test", () => {
      for (
        let i = rs2.option.OPTION_BACKLIGHT_COMPENSATION;
        i < rs2.option.OPTION_COUNT;
        i++
      ) {
        ...
        assert.equal(
          supports === undefined || typeof supports === "boolean",
          true
        );
...
describe("Sensor tests", function () {
  ...
  it("camera info", () => {
    sensors.forEach((sensor) => {
      for (
        let i = rs2.camera_info.CAMERA_INFO_NAME;
        i < rs2.camera_info.CAMERA_INFO_COUNT;
        i++
      ) {
  ...
  it("Stream profiles", () => {
    ...
    profiles0.forEach((p) => {
      ...
      assert.equal(p instanceof rs2.StreamProfile, true);
      assert.equal(
        p.streamType >= rs2.stream.STREAM_DEPTH &&
          p.streamType < rs2.stream.STREAM_COUNT,
        true
      );
      assert.equal(
        p.format >= rs2.format.FORMAT_Z16 && p.format < rs2.format.FORMAT_COUNT,
        true
      );
...
if (!(isRecord || isPlayback)) {
  describe("record & playback test", function () {
      ...
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          ...
          for (let i = 0; i < profiles.length; i++) {
            if (
              profiles[i].streamType === rs2.stream.STREAM_DEPTH &&
              profiles[i].fps === 30 &&
    ...
    it("playback test", () => {
      return new Promise((resolve, reject) => {
        startRecording(fileName, 1, null)
          .then(() => {
            assert.equal(fs.existsSync(fileName), true);
